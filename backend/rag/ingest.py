...
def parse_markdown_to_sections(file_path: str) -> list[dict[str, str]]:
    ...
    Returns:
        list[dict[str, str]]: List of dictionaries containing section headers
            and text content.
    """
    if not os.path.exists(file_path):
        logger.warning("Warning: file %s does not exist.", file_path)
        return []
...
def build_vector_db(kb_dir: str = KB_DIR, db_dir: str = DB_DIR) -> None:
    ...
    """
    logger.info("Initializing ChromaDB persistent client at: %s", db_dir)
    ...
    if existing_count > 0:
        logger.info(
            "Clearing %s existing items from vector database...",
            existing_count,
        )
    ...
    if documents:
        logger.info(
            "Adding %d document chunks to vector database...", len(documents)
        )
        collection.add(documents=documents, metadatas=metadatas, ids=ids)
        logger.info("Ingestion completed successfully.")
    else:
        logger.warning("No content chunks found to ingest.")
